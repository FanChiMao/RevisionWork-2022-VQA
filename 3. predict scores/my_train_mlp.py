...
if __name__ == '__main__':
    ...
    for iteration in range(iterations):
        ...
        for model_ in range(0, fold_number):
            ...
            if model_ == 9:  # last fold (把不整除的資料處理掉)
                ...
                for i, each_shuffle_data in enumerate(data_shuffle):
                    X_train.append(np.vstack((each_shuffle_data[0:data_divide * model_],
                                              each_shuffle_data[data_divide * (1 + model_) - 1:])))  # data_divide = 120
                    X_test.append((each_shuffle_data[0 + data_divide * model_:data_divide * (1 + model_) - 1]))
                Y_train = np.vstack(
                    (label_shuffle[0:data_divide * model_], label_shuffle[data_divide * (1 + model_) - 1:]))
            ...
            for channel in range(0, 7):
                ...
                r_max = 0
                print('==== Iteration {} Model {} Channel {} ===='.format(iteration + 1, model_ + 1, channel + 1))
                history = model.fit(
                            [np.vstack(
                                (X_train[r][0:vali_data_divide * (model_ % 9)],
                                 X_train[r][vali_data_divide * (1 + (model_ % 9)):])),
                                np.vstack(
                                    (X_train[c][0:vali_data_divide * (model_ % 9)],
                                     X_train[c][vali_data_divide * (1 + (model_ % 9)):])),
                                np.vstack(
                                    (X_train[s][0:vali_data_divide * (model_ % 9)],
                ...
                result = model.predict(
                        [np.vstack(
                            (
                            X_train[r][0:vali_data_divide * (model_)], X_train[r][vali_data_divide * (1 + (model_)):])),
                            np.vstack(
                                (X_train[c][0:vali_data_divide * (model_)],
                                 X_train[c][vali_data_divide * (1 + (model_)):])),
                            np.vstack((X_train[s][0:vali_data_divide * (model_)],
                                       X_train[s][vali_data_divide * (1 + (model_)):]))],
                        verbose=0)
                aaa = np.array(result)
                result = model.predict(
                        [X_train[r][0 + vali_data_divide * (model_):vali_data_divide * (1 + (model_))],
                         X_train[c][0 + vali_data_divide * (model_):vali_data_divide * (1 + (model_))],
                         X_train[s][0 + vali_data_divide * (model_):vali_data_divide * (1 + (model_))]],
                        verbose=0)
                ...
                r_p__, pval_p = pearsonr(aaa[:, 0], np.hstack(
                        (Y_train[0:vali_data_divide * (model_)], Y_train[vali_data_divide * (1 + (model_)):])))
                r_s__, pval_s = spearmanr(aaa[:, 0], np.hstack(
                        (Y_train[0:vali_data_divide * (model_)], Y_train[vali_data_divide * (1 + (model_)):])))
                r_p_, pval_p = pearsonr(aa[:, 0],
                                            Y_train[0 + vali_data_divide * (model_):vali_data_divide * (1 + (model_))])
                r_s_, pval_s = spearmanr(aa[:, 0],
                                             Y_train[0 + vali_data_divide * (model_):vali_data_divide * (1 + (model_))])
                ...
                if r_mean > r_max:
                    r_max = r_mean
                    predict_train_score[:, channel:(channel + 1)] = aaa
                    predict_test_score[:, channel:(channel + 1)] = a
        ...
        sio.savemat(
            'checkpoints/KoNViD/result_data_update_epochs_100.mat',
